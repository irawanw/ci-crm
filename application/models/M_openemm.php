 ...
 * Time:
 */
class M_openemm extends MY_Model
{
    ...
    public function get_champs($type, $data = null)
    {
        $champs = array(
            'read.parent'  => array(
                ...
                array('openemm_id', 'ref', "Openemm#", 'openemm', 'openemm_id', 'openemm_id'),
                //PARAMETRES
                ...
                /*array('openemm_number_of_open', 'text', "Open", 'openemm_number_of_open'),
                array('openemm_open_rate', 'text', "%Open", 'openemm_open_rate'),*/
                array('openemm_number_of_click', 'text', "Click", 'openemm_number_of_click'),
                array('openemm_click_rate', 'text', "%Click", 'openemm_click_rate'),
                //array('openemm_id', 'text', "Quantité envoyée", 'quantite_envoyee'), //
            ...
            'write.child'  => array(
                ...
                'openemm'                  => array("Openemm", 'select', array('openemm', 'id', 'value'), false),
    ...
    public function liste($void, $limit = 10, $offset = 1, $filters = null, $ordercol = 2, $ordering = "asc")
    {
        ...
        $openemm_id = $table . ".openemm_id as RowID";
        $checkbox   = $table . ".openemm_id as checkbox";
        ...
        $message_view              = "CONCAT('<a href=\"#\" class=\"view-text\" data-id=\"',t_openemm.message,'\" data-message=\"',t_message.message,'\">','Voir Message','</a>') as message_view";
        ...
        $open                      = "(select SUM(open) from t_openemm_child where parent_id = openemm_id AND inactive IS NULL AND deleted IS NULL) as open";
        //$open_pourcentage = "'' as open_pourcentage";
        $open_pourcentage = "(select ROUND(AVG(open_pourcentage),1) from t_openemm_child where parent_id = openemm_id AND inactive IS NULL AND deleted IS NULL) as open_pourcentage";
        $quantite_envoyee = "(select SUM(quantite_envoyee) from t_openemm_child where parent_id = openemm_id AND inactive IS NULL AND deleted IS NULL) as quantite_envoyee";
        ...
        //$this->db->group_by($table . '.openemm_id');
        ...
        $id = intval($void);
        if ($id > 0) {
            $this->db->where($table . '.openemm_id', $id);
        }
        ...
        $aliases = array(
            ...
            'openemm_current'           => "tos.current",
            'openemm_number_of_open'    => "tos.number_of_open",
            'openemm_open_rate'         => "CONCAT(tos.open_rate * 100 , '%')",
            'openemm_number_of_click'   => "tos.number_of_click",
            'openemm_click_rate'        => "CONCAT(tos.click_rate * 100 , '%')",
        );
        ...
        for ($i = 0; $i < count($resultat['data']); $i++) {
            $resultat['data'][$i]->checkbox                 = '<input type="checkbox" name="ids[]" value="' . $resultat['data'][$i]->openemm_id . '">';
            ...
            $resultat['data'][$i]->view_detail = '<a href="#" data-id="' . $resultat['data'][$i]->openemm_id . '" class="btn-view-detail">view detail</a>';
    ...
    public function liste_child($void, $parent_id = 0, $limit = 10, $offset = 1, $filters = null, $ordercol = 2, $ordering = "asc")
    {
        ...
        $message_commercial        = "t_salesman.emp_nom as message_commercial";
        $ht                        = "v_fac.total_ht AS ht";
        $openemm_current           = "tos.current as openemm_current";
        $openemm_number_of_open    = "tos.number_of_open as openemm_number_of_open";
        $openemm_open_rate         = "CONCAT(tos.open_rate * 100 , '%') as openemm_open_rate";
        $openemm_number_of_click   = "tos.number_of_click as openemm_number_of_click";
        $openemm_click_rate        = "CONCAT(tos.click_rate * 100 , '%') as openemm_click_rate";

        $this->db->select($table . ".*, t_openemm_child.*,
                $checkbox,
                $openemm_id,
                $client_name,
                $commande_name,
                facture_name,
                software_nom,
                t_openemm_child.open,
                t_openemm_child.open_pourcentage,
                $ht,
                $operateur_qui_envoie_name,
                $message_numero,
                $message_name,
                ...
                $openemm_click_rate
            ", false);
        $this->db->join('t_openemm_child', 'parent_id=' . $table . '.openemm_id', 'inner');
        $this->db->join('t_softwares', 'software=software_id', 'left');
        $this->db->join('t_contacts', 'ctc_id=client', 'left');
        $this->db->join('t_commandes', 'cmd_id=commande', 'left');
        $this->db->join('t_articles_devis', 'ard_devis=cmd_devis', 'left');
        ...
        $this->db->join('t_utilisateurs as t_util', 't_message.salesman = t_util.utl_id', 'left');
        $this->db->join('t_employes as t_salesman', 't_salesman.emp_id = t_util.utl_id', 'left');
        $this->db->join('t_openemm_stats as tos', 't_openemm_child.openemm=tos.openemm_id', 'left');
        ...
        switch ($void) {
            case 'archived':
                $this->db->where('t_openemm_child.inactive IS NOT NULL');
                break;
            case 'deleted':
                $this->db->where('t_openemm_child.deleted IS NOT NULL');
                break;
            case 'all':
                break;
            default:
                $this->db->where('t_openemm_child.inactive is NULL');
                $this->db->where('t_openemm_child.deleted is NULL');
                break;
        }

        $id = intval($void);
        if ($id > 0) {
            $this->db->where('openemm_child_id', $id);
        }
    ...
    public function detail($id)
    {
        ...
        $this->db->where('openemm_id = "' . $id . '"');
        $q = $this->db->get('t_openemm as tmb');
        if ($q->num_rows() > 0) {
            $resultat = $q->row();
            //get facture ht
    ...
    /******************************
     * New Livraisons insert into t_openemm table
     ******************************/
    public function nouveau($data)
    {
        return $this->_insert('t_openemm', $data);
    }

    public function nouveau_child($data)
    {
        return $this->_insert('t_openemm_child', $data);
    }
    ...
    {
        return $this->_update('t_openemm', $data, $id, 'openemm_id');
    }

    public function maj_child($data, $id)
    {
        return $this->_update('t_openemm_child', $data, $id, 'openemm_child_id');
    }
    ...
    {
        return $this->_delete('t_openemm', $id, 'openemm_id', 'inactive');
    }

    public function archive_child($id)
    {
        return $this->_delete('t_openemm_child', $id, 'openemm_child_id', 'inactive');
    }
    ...
    {
        return $this->_delete('t_openemm', $id, 'openemm_id', 'deleted');
    }

    public function remove_child($id)
    {
        return $this->_delete('t_openemm_child', $id, 'openemm_child_id', 'deleted');
    }
    ...
    {
        $query = $this->db->select("openemm_id as id, CONCAT(openemm_id,'-',t_message.name) as value")
            ->join('t_message_list as t_message', 't_message.message_list_id=t_openemm.message', 'left')
            ->where('t_openemm.inactive is NULL')
            ->where('t_openemm.deleted is NULL')
            ->get('t_openemm');

        return $query->result();
    }

    public function commande($openemm_id)
    {
        $this->db->select("
                tc.cmd_id,
                tc.cmd_reference
        ...
        $this->db->where('tmb.openemm_id = "' . $openemm_id . '"');
        $q                         = $this->db->get('t_openemm tmb');
        $resultat                  = $q->result();
    ...
    {
        $query = $this->db->get_where('t_openemm_stats', array('openemm_id' => $id));
