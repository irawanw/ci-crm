...
class M_sendgrid extends MY_Model
{
    ...
    public function get_champs($type, $data = null)
    {
        $champs = array(
            'read.parent'  => array(
                ...
                array('sendgrid_id', 'ref', "Sendgrid#", 'sendgrid', 'sendgrid_id', 'sendgrid_id'),
                //PARAMETRES
                ...
                /*array('sendgrid_number_of_open', 'text', "Open", 'sendgrid_number_of_open'),
                array('sendgrid_open_rate', 'text', "%Open", 'sendgrid_open_rate'),*/
                array('sendgrid_number_of_click', 'text', "Click", 'sendgrid_number_of_click'),
                array('sendgrid_click_rate', 'text', "%Click", 'sendgrid_click_rate'),
    ...
    public function liste($void, $limit = 10, $offset = 1, $filters = null, $ordercol = 2, $ordering = "asc")
    {
        ...
        $sendgrid_id = $table . ".sendgrid_id as RowID";
        $checkbox    = $table . ".sendgrid_id as checkbox";

        //$date_sendgrid = formatte_sql_date("date_sendgrid");
        ...
        $message_view              = "CONCAT('<a href=\"#\" class=\"view-text\" data-id=\"',t_sendgrid.message,'\" data-message=\"',t_message.message,'\">','Voir Message','</a>') as message_view";
        $message_commercial        = "t_salesman.emp_nom as message_commercial";
        $ht                        = "v_fac.total_ht AS ht";
        $sendgrid_current          = "'0' as sendgrid_current";
        $sendgrid_number_of_open   = "'0' as sendgrid_number_of_open";
        $sendgrid_open_rate        = "'0' as sendgrid_open_rate";
        $sendgrid_number_of_click  = "'0' as sendgrid_number_of_click";
        $sendgrid_click_rate       = "'0' as sendgrid_click_rate";
        ...
        $open                      = "(select SUM(open) from t_sendgrid_child where parent_id = sendgrid_id AND inactive IS NULL AND deleted IS NULL) as open";
        //$open_pourcentage = "'' as open_pourcentage";
        $open_pourcentage = "(select ROUND(AVG(open_pourcentage),1) from t_sendgrid_child where parent_id = sendgrid_id AND inactive IS NULL AND deleted IS NULL) as open_pourcentage";
        $quantite_envoyee = "(select SUM(quantite_envoyee) from t_sendgrid_child where parent_id = sendgrid_id AND inactive IS NULL AND deleted IS NULL) as quantite_envoyee";
        ...
        $this->db->select($table . ".*,
                ...
                $message_commercial,
                $sendgrid_current,
                $sendgrid_number_of_open,
                $sendgrid_open_rate,
                $sendgrid_number_of_click,
                $sendgrid_click_rate,
                $quantite_envoyee,
                $segment_part,
                $stats,
        ...
        //$this->db->group_by($table . '.sendgrid_id');
        ...
        $id = intval($void);
        if ($id > 0) {
            $this->db->where($table . '.sendgrid_id', $id);
        }
        ...
        $aliases = array(
            ...
            'sendgrid_current'          => "tos.current",
            'sendgrid_number_of_open'   => "tos.number_of_open",
            'sendgrid_open_rate'        => "CONCAT(tos.open_rate * 100 , '%')",
            'sendgrid_number_of_click'  => "tos.number_of_click",
            'sendgrid_click_rate'       => "CONCAT(tos.click_rate * 100 , '%')",
        );
        ...
        for ($i = 0; $i < count($resultat['data']); $i++) {
            $resultat['data'][$i]->checkbox                 = '<input type="checkbox" name="ids[]" value="' . $resultat['data'][$i]->sendgrid_id . '">';
            ...
            $resultat['data'][$i]->view_detail = '<a href="#" data-id="' . $resultat['data'][$i]->sendgrid_id . '" class="btn-view-detail">view detail</a>';
    ...
    public function liste_child($void, $parent_id = 0, $limit = 10, $offset = 1, $filters = null, $ordercol = 2, $ordering = "asc")
    {
        ...
        $message_commercial        = "t_salesman.emp_nom as message_commercial";
        $ht                        = "v_fac.total_ht AS ht";
        $sendgrid_current          = "tos.current as sendgrid_current";
        $sendgrid_number_of_open   = "tos.number_of_open as sendgrid_number_of_open";
        $sendgrid_open_rate        = "CONCAT(tos.open_rate * 100 , '%') as sendgrid_open_rate";
        $sendgrid_number_of_click  = "tos.number_of_click as sendgrid_number_of_click";
        $sendgrid_click_rate       = "CONCAT(tos.click_rate * 100 , '%') as sendgrid_click_rate";

        $this->db->select($table . ".*, t_sendgrid_child.*,
                $checkbox,
                $sendgrid_id,
                $client_name,
                $commande_name,
                facture_name,
                software_nom,
                t_sendgrid_child.open,
                t_sendgrid_child.open_pourcentage,
                $ht,
                $operateur_qui_envoie_name,
                $message_numero,
                $message_name,
                ...
                $sendgrid_click_rate
            ", false);
        $this->db->join('t_sendgrid_child', 'parent_id=' . $table . '.sendgrid_id', 'inner');
        $this->db->join('t_softwares', 'software=software_id', 'left');
        $this->db->join('t_contacts', 'ctc_id=client', 'left');
        $this->db->join('t_commandes', 'cmd_id=commande', 'left');
        $this->db->join('t_articles_devis', 'ard_devis=cmd_devis', 'left');
        ...
        $this->db->join('t_utilisateurs as t_util', 't_message.salesman = t_util.utl_id', 'left');
        $this->db->join('t_employes as t_salesman', 't_salesman.emp_id = t_util.utl_id', 'left');
        $this->db->join('t_sendgrid_stats as tos', 't_sendgrid_child.sendgrid=tos.sendgrid_id', 'left');
        ...
        switch ($void) {
            case 'archived':
                $this->db->where('t_sendgrid_child.inactive IS NOT NULL');
                break;
            case 'deleted':
                $this->db->where('t_sendgrid_child.deleted IS NOT NULL');
                break;
            case 'all':
                break;
            default:
                $this->db->where('t_sendgrid_child.inactive is NULL');
                $this->db->where('t_sendgrid_child.deleted is NULL');
                break;
        }

        $id = intval($void);
        if ($id > 0) {
            $this->db->where('sendgrid_child_id', $id);
        }
    ...
    public function liste_filterable_columns()
    {
        $filterable_columns = array(
            ...
            'rotation'                  => 'char',
            'quantite_envoyer'          => 'int',
            'sendgrid_current'          => 'int',
            'sendgrid_number_of_open'   => 'int',
            'sendgrid_open_rate'        => 'int',
            'sendgrid_number_of_click'  => 'int',
            'sendgrid_click_rate'       => 'int',
            'open'                      => 'int',
    ...
    public function detail($id)
    {
        ...
        $this->db->where('sendgrid_id = "' . $id . '"');
        $q = $this->db->get('t_sendgrid as tmb');
        if ($q->num_rows() > 0) {
            $resultat = $q->row();
            //get facture ht
            ...
            //get sendgrid data
            $sendgrid_data                      = $this->get_mailing_detail($resultat->sendgrid);
            $resultat->sendgrid_name            = $sendgrid_data['sendgrid_name'];
            $resultat->sendgrid_current         = $sendgrid_data['sendgrid_current'];
            $resultat->sendgrid_number_of_open  = $sendgrid_data['sendgrid_number_of_open'];
            $resultat->sendgrid_open_rate       = ($sendgrid_data['sendgrid_open_rate'] * 100) . '%';
            $resultat->sendgrid_number_of_click = $sendgrid_data['sendgrid_number_of_click'];
            $resultat->sendgrid_click_rate      = ($sendgrid_data['sendgrid_click_rate'] * 100) . '%';
    ...
    public function detail_for_form_child($id)
    {
        $this->db->select('*');
        $this->db->where('sendgrid_child_id = "' . $id . '"');
        $q = $this->db->get('t_sendgrid_child');
        if ($q->num_rows() > 0) {
            $resultat = $q->row();
            return $resultat;
        } else {
    ...
    /******************************
     * New Livraisons insert into t_sendgrid table
     ******************************/
    public function nouveau($data)
    {
        return $this->_insert('t_sendgrid', $data);
    }

    public function nouveau_child($data)
    {
        return $this->_insert('t_sendgrid_child', $data);
    }

    /******************************
     * Updating sendgrid data
     ******************************/
    public function maj($data, $id)
    {
        return $this->_update('t_sendgrid', $data, $id, 'sendgrid_id');
    }

    public function maj_child($data, $id)
    {
        return $this->_update('t_sendgrid_child', $data, $id, 'sendgrid_child_id');
    }

    /******************************
     * Archive sendgrid data
     ******************************/
    public function archive($id)
    {
        return $this->_delete('t_sendgrid', $id, 'sendgrid_id', 'inactive');
    }

    public function archive_child($id)
    {
        return $this->_delete('t_sendgrid_child', $id, 'sendgrid_child_id', 'inactive');
    }
    ...
    {
        return $this->_delete('t_sendgrid', $id, 'sendgrid_id', 'deleted');
    }

    public function remove_child($id)
    {
        return $this->_delete('t_sendgrid_child', $id, 'sendgrid_child_id', 'deleted');
    }
    ...
    {
        $query = $this->db->select("sendgrid_id as id, CONCAT(sendgrid_id,'-',t_message.name) as value")
            ->join('t_message_list as t_message', 't_message.message_list_id=t_sendgrid.message', 'left')
            ->where('t_sendgrid.inactive is NULL')
            ->where('t_sendgrid.deleted is NULL')
            ->get('t_sendgrid');

        return $query->result();
    }

    public function commande($sendgrid_id)
    {
        $this->db->select("
                tc.cmd_id,
                tc.cmd_reference
        ...
        $this->db->where('tmb.sendgrid_id = "' . $sendgrid_id . '"');
        $q                         = $this->db->get('t_sendgrid tmb');
        $resultat                  = $q->result();
    ...
    {
        $query = $this->db->get_where('t_sendgrid_stats', array('sendgrid_id' => $id));

        if ($query->row()) {
            $row  = $query->row();
            $data = array(
                'sendgrid_name'             => '',
                'sendgrid_open_rate'        => $row->open_rate,
                'sendgrid_bounce_rate'      => $row->bounce_rate,
                'sendgrid_hard_bounce_rate' => $row->hard_bounce_rate,
                'sendgrid_soft_bounce_rate' => $row->soft_bounce_rate,
                'sendgrid_click_rate'       => $row->click_rate,
                'sendgrid_number_of_open'   => $row->number_of_open,
                'sendgrid_number_of_click'  => $row->number_of_click,
                'sendgrid_total'            => $row->total,
                'sendgrid_current'          => $row->current,
                'sendgrid_opened'           => 0,
            );
...
/* End of file M_sendgrid.php */
/* Location: .//tmp/fz3temp-1/M_sendgrid.php */
