...
class M_sendinblue extends MY_Model
{
    ...
    public function get_champs($type, $data = null)
    {
        $champs = array(
            'read.parent'  => array(
                ...
                array('sendinblue_id', 'ref', "sendinblue#", 'sendinblue', 'sendinblue_id', 'sendinblue_id'),
                //PARAMETRES
                ...
                /*array('sendinblue_number_of_open', 'text', "Open", 'sendinblue_number_of_open'),
                array('sendinblue_open_rate', 'text', "%Open", 'sendinblue_open_rate'),*/
                array('sendinblue_number_of_click', 'text', "Click", 'sendinblue_number_of_click'),
                array('sendinblue_click_rate', 'text', "%Click", 'sendinblue_click_rate'),
    ...
    public function liste($void, $limit = 10, $offset = 1, $filters = null, $ordercol = 2, $ordering = "asc")
    {
        ...
        $sendinblue_id = $table . ".sendinblue_id as RowID";
        $checkbox      = $table . ".sendinblue_id as checkbox";

        //$date_sendinblue = formatte_sql_date("date_sendinblue");
        ...
        $message_view               = "CONCAT('<a href=\"#\" class=\"view-text\" data-id=\"',t_sendinblue.message,'\" data-message=\"',t_message.message,'\">','Voir Message','</a>') as message_view";
        $message_commercial         = "t_salesman.emp_nom as message_commercial";
        $ht                         = "v_fac.total_ht AS ht";
        $sendinblue_current         = "'0' as sendinblue_current";
        $sendinblue_number_of_open  = "'0' as sendinblue_number_of_open";
        $sendinblue_open_rate       = "'0' as sendinblue_open_rate";
        $sendinblue_number_of_click = "'0' as sendinblue_number_of_click";
        $sendinblue_click_rate      = "'0' as sendinblue_click_rate";
        ...
        $open                       = "(select SUM(open) from t_sendinblue_child where parent_id = sendinblue_id AND inactive IS NULL AND deleted IS NULL) as open";
        //$open_pourcentage = "'' as open_pourcentage";
        $open_pourcentage = "(select ROUND(AVG(open_pourcentage),1) from t_sendinblue_child where parent_id = sendinblue_id AND inactive IS NULL AND deleted IS NULL) as open_pourcentage";
        $quantite_envoyee = "(select SUM(quantite_envoyee) from t_sendinblue_child where parent_id = sendinblue_id AND inactive IS NULL AND deleted IS NULL) as quantite_envoyee";
        ...
        $this->db->select($table . ".*,
                ...
                $message_commercial,
                $sendinblue_current,
                $sendinblue_number_of_open,
                $sendinblue_open_rate,
                $sendinblue_number_of_click,
                $sendinblue_click_rate,
                $quantite_envoyee,
                $segment_part,
                $stats,
                $physical_server,
        ...
        //$this->db->group_by($table . '.sendinblue_id');
        ...
        $id = intval($void);
        if ($id > 0) {
            $this->db->where($table . '.sendinblue_id', $id);
        }
        ...
        $aliases = array(
            ...
            'sendinblue_current'         => "tos.current",
            'sendinblue_number_of_open'  => "tos.number_of_open",
            'sendinblue_open_rate'       => "CONCAT(tos.open_rate * 100 , '%')",
            'sendinblue_number_of_click' => "tos.number_of_click",
            'sendinblue_click_rate'      => "CONCAT(tos.click_rate * 100 , '%')",
        );
        ...
        for ($i = 0; $i < count($resultat['data']); $i++) {
            $resultat['data'][$i]->checkbox                 = '<input type="checkbox" name="ids[]" value="' . $resultat['data'][$i]->sendinblue_id . '">';
            ...
            $resultat['data'][$i]->view_detail = '<a href="#" data-id="' . $resultat['data'][$i]->sendinblue_id . '" class="btn-view-detail">view detail</a>';
    ...
    public function liste_child($void, $parent_id = 0, $limit = 10, $offset = 1, $filters = null, $ordercol = 2, $ordering = "asc")
    {
        ...
        $message_commercial         = "t_salesman.emp_nom as message_commercial";
        $ht                         = "v_fac.total_ht AS ht";
        $sendinblue_current         = "tos.current as sendinblue_current";
        $sendinblue_number_of_open  = "tos.number_of_open as sendinblue_number_of_open";
        $sendinblue_open_rate       = "CONCAT(tos.open_rate * 100 , '%') as sendinblue_open_rate";
        $sendinblue_number_of_click = "tos.number_of_click as sendinblue_number_of_click";
        $sendinblue_click_rate      = "CONCAT(tos.click_rate * 100 , '%') as sendinblue_click_rate";

        $this->db->select($table . ".*, t_sendinblue_child.*,
                $checkbox,
                $sendinblue_id,
                $client_name,
                $commande_name,
                facture_name,
                software_nom,
                t_sendinblue_child.open,
                t_sendinblue_child.open_pourcentage,
                $ht,
                $operateur_qui_envoie_name,
                $message_numero,
                $message_name,
                ...
                $sendinblue_click_rate
            ", false);
        $this->db->join('t_sendinblue_child', 'parent_id=' . $table . '.sendinblue_id', 'inner');
        $this->db->join('t_softwares', 'software=software_id', 'left');
        $this->db->join('t_contacts', 'ctc_id=client', 'left');
        $this->db->join('t_commandes', 'cmd_id=commande', 'left');
        $this->db->join('t_articles_devis', 'ard_devis=cmd_devis', 'left');
        ...
        $this->db->join('t_utilisateurs as t_util', 't_message.salesman = t_util.utl_id', 'left');
        $this->db->join('t_employes as t_salesman', 't_salesman.emp_id = t_util.utl_id', 'left');
        $this->db->join('t_sendinblue_stats as tos', 't_sendinblue_child.sendinblue=tos.sendinblue_id', 'left');
        ...
        switch ($void) {
            case 'archived':
                $this->db->where('t_sendinblue_child.inactive IS NOT NULL');
                break;
            case 'deleted':
                $this->db->where('t_sendinblue_child.deleted IS NOT NULL');
                break;
            case 'all':
                break;
            default:
                $this->db->where('t_sendinblue_child.inactive is NULL');
                $this->db->where('t_sendinblue_child.deleted is NULL');
                break;
        }

        $id = intval($void);
        if ($id > 0) {
            $this->db->where('sendinblue_child_id', $id);
        }
    ...
    public function liste_filterable_columns()
    {
        $filterable_columns = array(
            ...
            'rotation'                   => 'char',
            'quantite_envoyer'           => 'int',
            'sendinblue_current'         => 'int',
            'sendinblue_number_of_open'  => 'int',
            'sendinblue_open_rate'       => 'int',
            'sendinblue_number_of_click' => 'int',
            'sendinblue_click_rate'      => 'int',
            'open'                       => 'int',
    ...
    public function detail($id)
    {
        ...
        $this->db->where('sendinblue_id = "' . $id . '"');
        $q = $this->db->get('t_sendinblue as tmb');
        if ($q->num_rows() > 0) {
            $resultat = $q->row();
            //get facture ht
            ...
            //get sendinblue data
            $sendinblue_data                      = $this->get_mailing_detail($resultat->sendinblue);
            $resultat->sendinblue_name            = $sendinblue_data['sendinblue_name'];
            $resultat->sendinblue_current         = $sendinblue_data['sendinblue_current'];
            $resultat->sendinblue_number_of_open  = $sendinblue_data['sendinblue_number_of_open'];
            $resultat->sendinblue_open_rate       = ($sendinblue_data['sendinblue_open_rate'] * 100) . '%';
            $resultat->sendinblue_number_of_click = $sendinblue_data['sendinblue_number_of_click'];
            $resultat->sendinblue_click_rate      = ($sendinblue_data['sendinblue_click_rate'] * 100) . '%';
    ...
    public function detail_for_form_child($id)
    {
        $this->db->select('*');
        $this->db->where('sendinblue_child_id = "' . $id . '"');
        $q = $this->db->get('t_sendinblue_child');
        if ($q->num_rows() > 0) {
            $resultat = $q->row();
            return $resultat;
        } else {
    ...
    /******************************
     * New Livraisons insert into t_sendinblue table
     ******************************/
    public function nouveau($data)
    {
        return $this->_insert('t_sendinblue', $data);
    }

    public function nouveau_child($data)
    {
        return $this->_insert('t_sendinblue_child', $data);
    }

    /******************************
     * Updating sendinblue data
     ******************************/
    public function maj($data, $id)
    {
        return $this->_update('t_sendinblue', $data, $id, 'sendinblue_id');
    }

    public function maj_child($data, $id)
    {
        return $this->_update('t_sendinblue_child', $data, $id, 'sendinblue_child_id');
    }

    /******************************
     * Archive sendinblue data
     ******************************/
    public function archive($id)
    {
        return $this->_delete('t_sendinblue', $id, 'sendinblue_id', 'inactive');
    }

    public function archive_child($id)
    {
        return $this->_delete('t_sendinblue_child', $id, 'sendinblue_child_id', 'inactive');
    }
    ...
    {
        return $this->_delete('t_sendinblue', $id, 'sendinblue_id', 'deleted');
    }

    public function remove_child($id)
    {
        return $this->_delete('t_sendinblue_child', $id, 'sendinblue_child_id', 'deleted');
    }
    ...
    {
        $query = $this->db->select("sendinblue_id as id, CONCAT(sendinblue_id,'-',t_message.name) as value")
            ->join('t_message_list as t_message', 't_message.message_list_id=t_sendinblue.message', 'left')
            ->where('t_sendinblue.inactive is NULL')
            ->where('t_sendinblue.deleted is NULL')
            ->get('t_sendinblue');

        return $query->result();
    }

    public function commande($sendinblue_id)
    {
        $this->db->select("
                tc.cmd_id,
                tc.cmd_reference
        ...
        $this->db->where('tmb.sendinblue_id = "' . $sendinblue_id . '"');
        $q                         = $this->db->get('t_sendinblue tmb');
        $resultat                  = $q->result();
    ...
    {
        $query = $this->db->get_where('t_sendinblue_stats', array('sendinblue_id' => $id));

        if ($query->row()) {
            $row  = $query->row();
            $data = array(
                'sendinblue_name'             => '',
                'sendinblue_open_rate'        => $row->open_rate,
                'sendinblue_bounce_rate'      => $row->bounce_rate,
                'sendinblue_hard_bounce_rate' => $row->hard_bounce_rate,
                'sendinblue_soft_bounce_rate' => $row->soft_bounce_rate,
                'sendinblue_click_rate'       => $row->click_rate,
                'sendinblue_number_of_open'   => $row->number_of_open,
                'sendinblue_number_of_click'  => $row->number_of_click,
                'sendinblue_total'            => $row->total,
                'sendinblue_current'          => $row->current,
                'sendinblue_opened'           => 0,
            );
...
/* End of file M_sendinblue.php */
/* Location: .//tmp/fz3temp-1/M_sendinblue.php */
