 ...
 */
/**
 * Famille de catalogue générique
 */
class Famille_G extends Famille_catalogue {
    const CODE = 'G';
    ...
    public function exploite($id,$data) {
        ...
        foreach($data as $ligne) {
            ...
            if (count($ligne) < 7) {
                $erreurs[] = "Article $code_article : le catalogue Générique doit avoir au moins 7 colonnes";
                return $erreurs;
            }
            if ($ligne[4] != 'oui') {
                $erreurs[] = "Article $code_article : la colonne 'Sélectionnable' doit être 'oui'";
            }
            $infos = array(
                $ligne[6] // commentaire
            );
