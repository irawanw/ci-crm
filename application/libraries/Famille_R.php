 ...
 */
/**
 * Famille de catalogue remises
 */
class Famille_R extends Famille_catalogue {
    const CODE = 'R';
    ...
    public function exploite($id,$data) {
        ...
        foreach($data as $ligne) {
            ...
            if (count($ligne) < 7) {
                $erreurs[] = "Article $code_article : le catalogue Remises doit avoir au moins 7 colonnes";
                return $erreurs;
            }
            if (!is_numeric($ligne[3]) OR $ligne[3] < 0) {
                $erreurs[] = "Article $code_article : le taux de remise doit être un nombre positif ou null";
            }
