...
class Sendinblue extends MY_Controller {

	private $profil;
    private $_SOFTWARE_SENDINBLUE_ID = 6;
    private $barre_action         = array(
        array(
            "Nouveau"       => array('sendinblue/nouveau', 'plus', true, 'sendinblue_nouveau', null, array('form')),
            "Nouveau child" => array('sendinblue/nouveau_child', 'plus', true, 'sendinblue_nouveau_child', null, array('form')),
        ),
        array(
            //"Consulter" => array('*sendinblue/detail','eye-open',false,'sendinblue_detail'),
            "Consulter/Modifier" => array('sendinblue/modification', 'pencil', false, 'sendinblue_modification'),
            "Archiver"           => array('sendinblue/archive', 'folder-close', false, 'sendinblue_archiver', "Veuillez confirmer la archive du sendinblue"),
            "Supprimer"          => array('sendinblue/remove', 'trash', false, 'sendinblue_supprimer', "Veuillez confirmer la suppression du sendinblue"),
        ),
        array(
    ...
    {
        parent::__construct();
        $this->load->model(array('m_sendinblue', 'm_message_list', 'm_segments'));
    }
    ...
    public function liste($id = 0, $mode = 0)
    {
        // commandes globales
        $cmd_globales = array(
            //array("Nouvelle livraison","sendinblue/nouveau",'default')
        );
        ...
        // descripteur
        $descripteur = array(
            'datasource'         => 'sendinblue/index',
            'detail'             => array('sendinblue/detail', 'sendinblue_id', 'description'),
            'archive'            => array('sendinblue/archive', 'sendinblue_id', 'archive'),
            'champs'             => $this->m_sendinblue->get_champs('read','parent'),
            'filterable_columns' => $this->m_sendinblue->liste_filterable_columns(),
        );

        //determine json script that will be loaded
        //for eg: sendinblue/archived_json in kendo_grid-js
        switch ($mode) {
            case 'archiver':
                $descripteur['datasource'] = 'sendinblue/archived';
                break;
            case 'supprimees':
                $descripteur['datasource'] = 'sendinblue/deleted';
                break;
            case 'all':
                $descripteur['datasource'] = 'sendinblue/all';
                break;
        }
        ...
        $scripts[] = $this->load->view("templates/datatables-js",
            array(
                ...
                'toolbar'             => $toolbar,
                'controleur'          => 'sendinblue',
                'methode'             => 'index',
                'mass_action_toolbar' => true,
                'view_toolbar'        => true,
            ), true);
        $scripts[] = $this->load->view("sendinblue/liste-js", array(), true);
        $scripts[] = $this->load->view('sendinblue/form-js', array(), true);

        // listes personnelles
        $vues = $this->m_vues->vues_ctrl('sendinblue', $this->session->id);
        $data = array(
            'title'        => "Envois Sendinblue",
            'page'         => "templates/datatables",
            'menu'         => "Extra|Sendinblue",
            'scripts'      => $scripts,
    ...
    public function index_json($id = 0)
    {
        ...
        if (empty($order) || empty($columns)) {
            ...
            $resultat = $this->m_sendinblue->liste($id, $pagelength, $pagestart, $filters);
        } else {
            ...
            if ($order_col_id >= 0 && $order_col_id <= count($columns)) {
                ...
                $resultat = $this->m_sendinblue->liste($id, $pagelength, $pagestart, $filters, $order_col, $ordering);
            } else {
                $resultat = $this->m_sendinblue->liste($id, $pagelength, $pagestart, $filters);
            }
        }
    ...
    public function index_child_json($id = 0)
    {
        ...
        if (empty($order) || empty($columns)) {
            ...
            //list with default ordering
            $resultat = $this->m_sendinblue->liste_child($id, $parent_id, $pagelength, $pagestart, $filters);
        } else {
            ...
            // tables for LINK columns
            $tables = array(
                'sendinblue_child_id' => 't_sendinblue_child',
            );
            if ($order_col_id >= 0 && $order_col_id <= count($columns)) {
                $order_col = $columns[$order_col_id]['data'];
                if (empty($order_col)) {
                ...
                $resultat = $this->m_sendinblue->liste_child($id, $parent_id, $pagelength, $pagestart, $filters, $order_col, $ordering);
            } else {
                $resultat = $this->m_sendinblue->liste_child($id, $parent_id, $pagelength, $pagestart, $filters);
            }
        }
    ...
    public function nouveau($id = 0, $ajax = false)
    {
        ...
        if ($this->form_validation->run() and $validation) {
            ...
            // validation réussie
            $valeurs = array(
                'software'       => $this->_SOFTWARE_SENDINBLUE_ID,
            ...
            $resultat = $this->m_sendinblue->nouveau($valeurs);
            ...
            if ($resultat === false) {
                ...
                $this->my_set_action_response($ajax, true, "Sendinblue a été enregistré avec succès", 'info', $ajaxData);
            }
            if ($ajax) {
                return;
            }
            ...
            $valeurs->software         = $this->_SOFTWARE_SENDINBLUE_ID;
            ...
            $listes_valeurs->client         = $this->m_sendinblue->client_option();
            $listes_valeurs->commande       = $this->m_sendinblue->commande(0);
            $listes_valeurs->message        = $this->m_message_list->simple_list();
            $listes_valeurs->segment_numero = $this->m_segments->liste_option();

            // descripteur
            $descripteur = array(
                'champs'  => $this->m_sendinblue->get_champs('write','parent'),
                'onglets' => array(                    
            ...
            $data = array(
                'title'          => "Ajouter un nouveau sendinblue",
                'page'           => "templates/form",
                'menu'           => "Extra|Create Sendinblue",
                'values'         => $valeurs,
                'action'         => "création",
                'multipart'      => false,
    ...
    public function nouveau_child($id = 0, $ajax = false)
    {
        ...
        if ($this->form_validation->run() and $validation) {
            ...
            $resultat = $this->m_sendinblue->nouveau_child($valeurs);
            ...
            if ($resultat === false) {
                ...
                $this->my_set_action_response($ajax, true, "Sendinblue child a été enregistré avec succès", 'info', $ajaxData);
            }
            if ($ajax) {
                return;
            }
            ...
            $listes_valeurs->parent_id                = $this->m_sendinblue->parent_option();
            $listes_valeurs->stats                    = $this->m_sendinblue->stats_option();
            $listes_valeurs->operateur_qui_envoie     = $this->m_sendinblue->utilisateurs_option();
            $listes_valeurs->segment_part             = $this->m_sendinblue->segment_part_option();
            $delivrabilite_sur_test_option            = $this->m_sendinblue->delivrabilite_sur_test_option();
            ...
            $listes_valeurs->physical_server          = $this->m_sendinblue->physical_server_option();
            $listes_valeurs->smtp                     = $this->m_sendinblue->smtp_option();
            $listes_valeurs->rotation                 = $this->m_sendinblue->rotation_option();

            // descripteur
            $descripteur = array(
                'champs'  => $this->m_sendinblue->get_champs('write','child'),
                'onglets' => array(
    ...
    public function modification($id = 0, $ajax)
    {
        ...
        if ($this->form_validation->run() and $validation) {
            ...
            $resultat    = $this->m_sendinblue->maj($valeurs, $id);
            $redirection = 'sendinblue/detail/' . $id;
            if ($resultat === false) {
                $this->my_set_action_response($ajax, false);
            } else {
                if ($resultat == 0) {
                    ...
                    $message  = "Sendinblue a été modifié";
                    $ajaxData = array(
                        'event' => array(
                            'controleur' => $this->my_controleur_from_class(__CLASS__),
                            'type'       => 'recordchange',
            ...
            // validation en échec ou premier appel : affichage du formulaire
            $valeurs = $this->m_sendinblue->detail_for_form($id);
            ...
            $listes_valeurs->client         = $this->m_sendinblue->client_option();
            $listes_valeurs->commande       = $this->m_sendinblue->commande($id);
            $listes_valeurs->message        = $this->m_message_list->simple_list();
            $listes_valeurs->segment_numero = $this->m_segments->liste_option();
            ...
            $data = array(
                'title'          => "Modifier Sendinblue",
                'page'           => "templates/form",
                'menu'           => "Extra|Edit Sendinblue",
                'id'             => $id,
                'values'         => $valeurs,
                'action'         => "modif",
                'multipart'      => false,
    ...
    public function modification_child($id = 0, $ajax)
    {
        ...
        if ($this->form_validation->run() and $validation) {
            ...
            $resultat    = $this->m_sendinblue->maj_child($valeurs, $id);
            $redirection = 'sendinblue/detail/' . $id;
            if ($resultat === false) {
                $this->my_set_action_response($ajax, false);
            } else {
                if ($resultat == 0) {
                    ...
                    $message  = "Sendinblue child a été modifié";
                    $ajaxData = array(
                        'event' => array(
                            'controleur' => $this->my_controleur_from_class(__CLASS__),
                            'type'       => 'recordchange',
            ...
            // validation en échec ou premier appel : affichage du formulaire
            $valeurs = $this->m_sendinblue->detail_for_form_child($id);
            ...
            $data = array(
                ...
                'id'             => $id,
                'values'         => $valeurs,
                'form-id'        => "form-sendinblue-modification_child-1",
                'action'         => "modif",
                'multipart'      => false,
                'confirmation'   => 'Enregistrer',
                'controleur'     => 'sendinblue',
    ...
    public function archive($id = 0, $ajax = false)
    {
        ...
        $resultat = $this->m_sendinblue->archive($id);
        ...
        if ($resultat === false) {
            ...
            $this->my_set_action_response($ajax, true, "Sendinblue a été archivee", 'info', $ajaxData);
        }
    ...
    public function archive_child($id = 0, $ajax = false)
    {
        ...
        $resultat = $this->m_sendinblue->archive_child($id);
        ...
        if ($resultat === false) {
            ...
            $this->my_set_action_response($ajax, true, "Sendinblue child a été archivee", 'info', $ajaxData);
        }
    ...
    public function remove($id, $ajax = false)
    {
        ...
        $resultat = $this->m_sendinblue->remove($id);
        ...
        if ($resultat === false) {
            ...
            $this->my_set_action_response($ajax, true, "Sendinblue a été supprimé", 'info', $ajaxData);
        }
    ...
    public function remove_child($id, $ajax = false)
    {
        ...
        $resultat = $this->m_sendinblue->remove_child($id);
        ...
        if ($resultat === false) {
            ...
            $this->my_set_action_response($ajax, true, "Sendinblue child a été supprimé", 'info', $ajaxData);
        }
    ...
    public function mass_unremove()
    {
        $ids = json_decode($this->input->post('ids'), true); //convert json into array
        foreach ($ids as $id) {
            $resultat = $this->m_sendinblue->unremove($id);
        }
    }
    ...
    public function commande_option($id = 0)
    {
        ...
        $resultat = $this->m_sendinblue->commande_by_client($id);
    ...
    {
        $resultat = $this->m_sendinblue->facture_option($id);
    ...
    public function sendinblue_option($id = 0)
    {
        $data = $this->m_sendinblue->get_mailing_detail($id);
        echo json_encode($data);
    }
...
/* Location: .//tmp/fz3temp-1/Sendinblue.php */