 ...
 * Time:
 */
class Openemm extends MY_Controller
{
    private $profil;
    private $_SOFTWARE_OPENEMM_ID = 1;
    private $barre_action         = array(
        array(
            "Nouveau"       => array('openemm/nouveau', 'plus', true, 'openemm_nouveau', null, array('form')),
            "Nouveau child" => array('openemm/nouveau_child', 'plus', true, 'openemm_nouveau_child', null, array('form')),
        ),
        array(
            //"Consulter" => array('*openemm/detail','eye-open',false,'openemm_detail'),
            "Consulter/Modifier" => array('openemm/modification', 'pencil', false, 'openemm_modification'),
            "Archiver"           => array('openemm/archive', 'folder-close', false, 'openemm_archiver', "Veuillez confirmer la archive du openemm"),
            "Supprimer"          => array('openemm/remove', 'trash', false, 'openemm_supprimer', "Veuillez confirmer la suppression du openemm"),
        ),
        array(
    ...
    {
        parent::__construct();
        $this->load->model(array('m_openemm', 'm_message_list', 'm_segments'));
    }
    ...
    public function liste($id = 0, $mode = 0)
    {
        // commandes globales
        $cmd_globales = array(
            //array("Nouvelle livraison","openemm/nouveau",'default')
        );
        ...
        // descripteur
        $descripteur = array(
            'datasource'         => 'openemm/index',
            'detail'             => array('openemm/detail', 'openemm_id', 'description'),
            'archive'            => array('openemm/archive', 'openemm_id', 'archive'),
            'champs'             => $this->m_openemm->get_champs('read','parent'),
            'filterable_columns' => $this->m_openemm->liste_filterable_columns(),
        );

        //determine json script that will be loaded
        //for eg: openemm/archived_json in kendo_grid-js
        switch ($mode) {
            case 'archiver':
                $descripteur['datasource'] = 'openemm/archived';
                break;
            case 'supprimees':
                $descripteur['datasource'] = 'openemm/deleted';
                break;
            case 'all':
                $descripteur['datasource'] = 'openemm/all';
                break;
        }
        ...
        $scripts[] = $this->load->view("openemm/liste-js", array(), true);
        $scripts[] = $this->load->view('openemm/form-js', array(), true);

        // listes personnelles
        $vues = $this->m_vues->vues_ctrl('openemm', $this->session->id);
        $data = array(
            'title'        => "Envois Open Emm",
    ...
    public function index_json($id = 0)
    {
        ...
        if (empty($order) || empty($columns)) {
            ...
            $resultat = $this->m_openemm->liste($id, $pagelength, $pagestart, $filters);
        } else {
            ...
            if ($order_col_id >= 0 && $order_col_id <= count($columns)) {
                ...
                $resultat = $this->m_openemm->liste($id, $pagelength, $pagestart, $filters, $order_col, $ordering);
            } else {
                $resultat = $this->m_openemm->liste($id, $pagelength, $pagestart, $filters);
            }
        }
    ...
    public function index_child_json($id = 0)
    {
        ...
        if (empty($order) || empty($columns)) {
            ...
            //list with default ordering
            $resultat = $this->m_openemm->liste_child($id, $parent_id, $pagelength, $pagestart, $filters);
        } else {
            ...
            // tables for LINK columns
            $tables = array(
                'openemm_child_id' => 't_openemm_child',
            );
            if ($order_col_id >= 0 && $order_col_id <= count($columns)) {
                $order_col = $columns[$order_col_id]['data'];
                if (empty($order_col)) {
                ...
                $resultat = $this->m_openemm->liste_child($id, $parent_id, $pagelength, $pagestart, $filters, $order_col, $ordering);
            } else {
                $resultat = $this->m_openemm->liste_child($id, $parent_id, $pagelength, $pagestart, $filters);
            }
        }
    ...
    public function nouveau($id = 0, $ajax = false)
    {
        ...
        if ($this->form_validation->run() and $validation) {
            ...
            $resultat = $this->m_openemm->nouveau($valeurs);
            ...
            if ($resultat === false) {
                ...
                $this->my_set_action_response($ajax, true, "Openemm a été enregistré avec succès", 'info', $ajaxData);
            }
            if ($ajax) {
                return;
            }
            ...
            $listes_valeurs->client         = $this->m_openemm->client_option();
            $listes_valeurs->commande       = $this->m_openemm->commande(0);
            $listes_valeurs->message        = $this->m_message_list->simple_list();
            $listes_valeurs->segment_numero = $this->m_segments->liste_option();

            // descripteur
            $descripteur = array(
                'champs'  => $this->m_openemm->get_champs('write','parent'),
                'onglets' => array(                    
            ...
            $data = array(
                'title'          => "Ajouter un nouveau Openemm",
                'page'           => "templates/form",
                'menu'           => "Extra|Create Openemm",
                'values'         => $valeurs,
                'action'         => "création",
                'multipart'      => false,
    ...
    public function nouveau_child($id = 0, $ajax = false)
    {
        ...
        if ($this->form_validation->run() and $validation) {
            ...
            $valeurs = array(
                ...
                'openemm'                  => $this->input->post('openemm'),
            ...
            $resultat = $this->m_openemm->nouveau_child($valeurs);
            ...
            if ($resultat === false) {
                ...
                $this->my_set_action_response($ajax, true, "Openemm child a été enregistré avec succès", 'info', $ajaxData);
            }
            if ($ajax) {
                return;
            }
            ...
            $valeurs->openemm                  = $this->input->post('openemm');
            ...
            $listes_valeurs->parent_id                = $this->m_openemm->parent_option();
            $listes_valeurs->stats                    = $this->m_openemm->stats_option();
            $listes_valeurs->operateur_qui_envoie     = $this->m_openemm->utilisateurs_option();
            $listes_valeurs->segment_part             = $this->m_openemm->segment_part_option();
            $listes_valeurs->openemm                  = $this->m_openemm->get_mailing();
            $delivrabilite_sur_test_option            = $this->m_openemm->delivrabilite_sur_test_option();
            ...
            $listes_valeurs->physical_server          = $this->m_openemm->physical_server_option();
            $listes_valeurs->smtp                     = $this->m_openemm->smtp_option();
            $listes_valeurs->rotation                 = $this->m_openemm->rotation_option();

            // descripteur
            $descripteur = array(
                'champs'  => $this->m_openemm->get_champs('write','child'),
                'onglets' => array(
                    array("Parent", array('parent_id')),
                    array("Segment", array('segment_part')),
                    array("Suivi de l'Envoi", array('date_envoi','stats', 'openemm','quantite_envoyee',"open","open_pourcentage")),
    ...
    public function modification($id = 0, $ajax)
    {
        ...
        if ($this->form_validation->run() and $validation) {
            ...
            $resultat    = $this->m_openemm->maj($valeurs, $id);
            $redirection = 'openemm/detail/' . $id;
            if ($resultat === false) {
                $this->my_set_action_response($ajax, false);
            } else {
                if ($resultat == 0) {
            ...
            // validation en échec ou premier appel : affichage du formulaire
            $valeurs = $this->m_openemm->detail_for_form($id);
            ...
            $listes_valeurs->client         = $this->m_openemm->client_option();
            $listes_valeurs->commande       = $this->m_openemm->commande($id);
            $listes_valeurs->message        = $this->m_message_list->simple_list();
            $listes_valeurs->segment_numero = $this->m_segments->liste_option();
            ...
            $data = array(
                'title'          => "Modifier Openemm",
                'page'           => "templates/form",
                'menu'           => "Extra|Edit Openemm",
                'id'             => $id,
                'values'         => $valeurs,
                'action'         => "modif",
                'multipart'      => false,
    ...
    public function modification_child($id = 0, $ajax)
    {
        ...
        if ($this->form_validation->run() and $validation) {
            ...
            $resultat    = $this->m_openemm->maj_child($valeurs, $id);
            $redirection = 'openemm/detail/' . $id;
            if ($resultat === false) {
                $this->my_set_action_response($ajax, false);
            } else {
                if ($resultat == 0) {
                    ...
                    $message  = "Openemm child a été modifié";
                    $ajaxData = array(
                        'event' => array(
                            'controleur' => $this->my_controleur_from_class(__CLASS__),
                            'type'       => 'recordchange',
            ...
            // validation en échec ou premier appel : affichage du formulaire
            $valeurs = $this->m_openemm->detail_for_form_child($id);
            ...
            $data = array(
                ...
                'id'             => $id,
                'values'         => $valeurs,
                'form-id'        => "form-openemm-modification_child-1",
                'action'         => "modif",
                'multipart'      => false,
                'confirmation'   => 'Enregistrer',
                'controleur'     => 'openemm',
    ...
    public function archive($id = 0, $ajax = false)
    {
        ...
        $resultat = $this->m_openemm->archive($id);
        ...
        if ($resultat === false) {
            ...
            $this->my_set_action_response($ajax, true, "Openemm a été archivee", 'info', $ajaxData);
        }
    ...
    public function archive_child($id = 0, $ajax = false)
    {
        ...
        $resultat = $this->m_openemm->archive_child($id);
        ...
        if ($resultat === false) {
            ...
            $this->my_set_action_response($ajax, true, "Openemm child a été archivee", 'info', $ajaxData);
        }
    ...
    public function remove($id, $ajax = false)
    {
        ...
        $resultat = $this->m_openemm->remove($id);
        ...
        if ($resultat === false) {
            ...
            $this->my_set_action_response($ajax, true, "Openemm a été supprimé", 'info', $ajaxData);
        }
    ...
    public function remove_child($id, $ajax = false)
    {
        ...
        $resultat = $this->m_openemm->remove_child($id);
        ...
        if ($resultat === false) {
            ...
            $this->my_set_action_response($ajax, true, "Openemm child a été supprimé", 'info', $ajaxData);
        }
    ...
    public function mass_unremove()
    {
        $ids = json_decode($this->input->post('ids'), true); //convert json into array
        foreach ($ids as $id) {
            $resultat = $this->m_openemm->unremove($id);
        }
    }
    ...
    public function commande_option($id = 0)
    {
        ...
        $resultat = $this->m_openemm->commande_by_client($id);
    ...
    {
        $resultat = $this->m_openemm->facture_option($id);
    ...
    public function openemm_option($id = 0)
    {
        $data = $this->m_openemm->get_mailing_detail($id);
        echo json_encode($data);
    }
}
// EOF
