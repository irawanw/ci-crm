...
<div id="modal-custom-remove" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la suppression du Sendgrid
      </div>
...
<div id="modal-custom-archive" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la archive du Sendgrid
      </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#datatable tbody').on('click', '.btn-view-detail', function(e) {
            ...
            if(isHide == false){
                if ( row.child.isShown() ) {
                    ...
                    //get child data
                    $.post('<?php echo site_url(); ?>/sendgrid/index_child_json/0', {'parentId':tr_id}, function(data){
                        ...
                        if(data.data.length > 0) {
                            $.each(data.data, function(index, value){                       
                                ...
                                new_html = '<tr id="child-id-'+value.sendgrid_child_id+'" data-parent="'+tr_id+'" class="child child-'+tr_id+'">'+
                                                ...
                                                '<td>'+value.sendgrid_number_of_click+'</td>'+
                                                '<td>'+value.sendgrid_click_rate+'</td>'+
                                ...
                                if ($('.child-'+tr_id).length == (index+1)) {
                                    ...
                                    $('#datatable tr').click(function(){
                                        ...
                                        $('#sendgrid_supprimer').addClass("action-confirm-danger");                            
                                        //modify button modification & remove
                                        if($(this).hasClass('child')){                                  
                                            child_id = $(this).attr('id').split('-');
                                            child_id = child_id[2];

                                            $("#sendgrid_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/sendgrid/modification_child/'+child_id);
                                            $("#sendgrid_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/sendgrid/archive_child/'+child_id);
                                            $("#sendgrid_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/sendgrid/remove_child/'+child_id);
                                            
                                        } else {
                                            $("#sendgrid_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/sendgrid/modification/'+parent_id);

                                            $("#sendgrid_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/sendgrid/remove/'+parent_id);

                                            $("#sendgrid_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/sendgrid/archive/'+parent_id);
                                        }
                                    })
                                }
                            })
        ...
        $("#sendgrid_supprimer a").click(function(ev){
            ev.preventDefault();
        ...
        $("#sendgrid_archiver a").click(function(ev){
            ev.preventDefault();
            var href = $(this).attr('href');
