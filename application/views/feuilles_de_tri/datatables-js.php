    ...
    <?php if (!isset($id)) {
            ...
            fnDrawCallback: function( settings ) {
                ...
                api.column(2, {page:'current'} ).data().each( function ( group, i ) {
                    var row_data = api.rows( {page:'current'} ).data();
                    var row = row_data[i];
                    var secteur_id = row.secteur_id;

                    if ( last !== group ) {
                        $(rows).eq( i ).before(
                            '<tr class="group"><td colspan="8"><input type="checkbox" name="secteur_ids[]" value="'+ secteur_id +'" /> '+group+'</td></tr>'
                        );
