    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <div class="modal-body">
                êtes vous certain de vouloir annuler la saisie du maxbulk ? 
            </div>
...
<div class="modal fade template-modal" id="modification_maxbulk" tabindex="-1" role="dialog" data-id="">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">				
				<div class="frm_modification_maxbulk">
...
<script>
    $(document).ready(function(){
	    ...
	    $("#modification_maxbulk").on("focus", "input.form-date-field",  function() {
	        $(this).datetimepicker({
	            format:'d/m/Y',
	            formatDate:'d/m/Y',
	            timepicker: false,
		...
		$("#template-modal-detail,#modification_maxbulk").on("change",'#client', function(){
			get_commande();			
		});
		...
		$("#modification_maxbulk").on('change',"#segment_numero", function() {			
		...
		$('#template-modal-detail').on('shown.bs.modal',function(e) {
			$('#form-max_bulk-nouveau-1').find('#segment_criteria').attr("readonly", true);						
			var formId = $(this).find('form').prop('id');	

			if(formId == "form-max_bulk-nouveau-1") {
				var btn = '<button type="button" class="btn btn-primary" id="enregister-suivant">Enregister / suivant</button>';
				var btnAnnuler = '&nbsp;<button class="btn btn-default" id="annuler-max-bulk">Annuler</button>';
				...
				var btnAnnulerChild = '&nbsp;<button class="btn btn-default" id="annuler-max-bulk-child">Annuler</button>';
				...
				setTimeout(function() {
					...
					if($("#annuler-max-bulk-child").length == 0) {
		...
		$('#template-modal-detail,#modification_maxbulk').on('hide.bs.modal',function(e) {	
			$('#enregister-suivant').remove();
			$('#enregister-suivant-child').remove();
			$('#annuler-max-bulk').remove();
			$('#annuler-max-bulk-child').remove();			
		});
		...
		$(document).on("click","#modification_maxbulk .modal-body li a",function()
	    {
	        tab = $(this).attr("href");
		 ...
		 * FORM EDIT
		 */
		$('#modification_maxbulk').on('shown.bs.modal',function(e) {
			$('#form-max_bulk-modification-1').find('#segment_criteria').attr("readonly", true);

			var formId = $(this).find('form').prop('id');					

			if(formId == "form-max_bulk-modification-1") {
				var btn = '<button type="button" class="btn btn-primary" id="enregister-suivant">Enregister / suivant</button>';
				var btnAnnuler = '&nbsp;<button class="btn btn-default" id="annuler-max-bulk">Annuler</button>';

				$('#modification_maxbulk button[type="submit"]').hide();
				setTimeout(function() {
					if($("#enregister-suivant").length == 0) {
						$('#modification_maxbulk form p.text-center').append(btn);
					}
					if($("#annuler-max-bulk").length == 0) {
						$('#modification_maxbulk form p.text-center').append(btnAnnuler);
					}
				}, 20);
			} else {
				var btnChild = '<button type="button" class="btn btn-primary" id="enregister-suivant-child">Enregister / suivant</button>';
				...
				setTimeout(function() {
					if($("#enregister-suivant-child").length == 0) {
						$('#modification_maxbulk form p.text-center').append(btnChild);
					}
					if($("#annuler-max-bulk-child").length == 0) {
						$('#modification_maxbulk form p.text-center').append(btnAnnulerChild);
					}
				}, 20);
			}

			setTimeout(function(){				
				get_segment_criteria($('#form-max_bulk-modification-1').find('#segment_numero').val(), true);
			},100);
		});

		$(document).on('click',"#modification_maxbulk #form-max_bulk-modification-1 #enregister-suivant",function(e) {
			var templateId = "#modification_maxbulk";
			...
			if(tabActive != "max_bulk-tab-4") {
				...
				$("#modification_maxbulk .modal-body .tab-content div").each(function(){
		            $(this).removeClass("active");
		        });
		        $("#modification_maxbulk #"+nextTab).addClass("active");
		...
		$(document).on('click',"#modification_maxbulk #enregister-suivant-child",function(e) {
			var templateId = "#modification_maxbulk";
			...
			if(tabActiveChild != "max_bulk-tab-5") {
		        ...
		        $("#modification_maxbulk #"+nextTabChild).addClass("active");
		...
		$('#modification_maxbulk').on('shown.bs.tab',"a",function(e) {
			var templateId = "#modification_maxbulk";
			var tab = $(templateId).find('ul.nav-tabs li.active a');
			var tabActive = $(tab).attr('aria-controls');
		...
		$('#modification_maxbulk').on('click',"#annuler-max-bulk,#annuler-max-bulk-child",function(e) {
			e.preventDefault();
			$('#modal-annuler-confirm').modal('show');
		});

		$('#max_bulk_modification a').click(function(e) {
			e.preventDefault();
			...
			$.ajax({
				...
				{
					$('.frm_modification_maxbulk').html(response.data);
					$('#modification_maxbulk').modal('show');			
				}
			});
		});

		$('#template-modal-detail').on("click","#form-submit-max_bulk-nouveau_child", function(e) {
			e.preventDefault();

			var form = $('#form-max_bulk-nouveau_child-1');			
		 ...
		 * 
		 */
		$("#modification_maxbulk").on("click","#form-submit-max_bulk-modification,#form-submit-max_bulk-modification_child", function(e) {
			e.preventDefault();
		
			var form = $('#modification_maxbulk').find('form');			
			var url = $(form).attr("action");
	        var data = $(form).serialize();	   
	            	...
	            	if(response.success == true) {
	            		...
	            		if(event){
		            		...
		            		if(!isChild) {
		            			...
		            			$.ajax({
		            				type: 'POST',
		            				url: "<?php echo site_url('max_bulk/index_child_json');?>/" + event.id,
	            		...
	            		$('#modification_maxbulk').modal('hide');
	            	}
	...
	function get_commande() {
		$.get("<?php echo site_url('max_bulk/commande_option');?>/"+$("#client").val(), function(data){
				$("#commande").html(data);
		});
	}	
	...
	function get_segment_criteria(segmentNumero, isUpdate) {
		$.get("<?php echo site_url('segments/get');?>/"+segmentNumero+"/criteria", function(response){
			if(isUpdate) {
				$("#form-max_bulk-modification-1").find('#segment_criteria').html(response.data);
			} else {
				$("#form-max_bulk-nouveau-1").find('#segment_criteria').html(response.data);
			}
		},"json");
	}
</script>
