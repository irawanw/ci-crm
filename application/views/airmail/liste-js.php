...
<div id="modal-custom-remove" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la suppression du Air mail
      </div>
...
<div id="modal-custom-archive" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la archive du Air mail
      </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#datatable tbody').on('click', '.btn-view-detail', function(e) {
            ...
            if(isHide == false){
                if ( row.child.isShown() ) {
                    ...
                    //get child data
                    $.post('<?php echo site_url(); ?>/airmail/index_child_json/0', {'parentId':tr_id}, function(data){
                        ...
                        if(data.data.length > 0) {
                            $.each(data.data, function(index, value){                       
                                ...
                                new_html = '<tr id="child-id-'+value.airmail_child_id+'" data-parent="'+tr_id+'" class="child child-'+tr_id+'">'+
                                                ...
                                                //'<td>'+value.airmail_number_of_open+'</td>'+
                                                //'<td>'+value.airmail_open_rate+'</td>'+
                                                '<td>'+value.airmail_number_of_click+'</td>'+
                                                '<td>'+value.airmail_click_rate+'</td>'+
                                ...
                                if ($('.child-'+tr_id).length == (index+1)) {
                                    ...
                                    $('#datatable tr').click(function(){
                                        ...
                                        $('#airmail_supprimer').addClass("action-confirm-danger");                            
                                        //modify button modification & remove
                                        if($(this).hasClass('child')){                                  
                                            child_id = $(this).attr('id').split('-');
                                            child_id = child_id[2];

                                            $("#airmail_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/airmail/modification_child/'+child_id);
                                            $("#airmail_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/airmail/archive_child/'+child_id);
                                            $("#airmail_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/airmail/remove_child/'+child_id);
                                            
                                        } else {
                                            $("#airmail_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/airmail/modification/'+parent_id);

                                            $("#airmail_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/airmail/remove/'+parent_id);

                                            $("#airmail_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/airmail/archive/'+parent_id);
                                        }
                                    })
                                }
                            })
        ...
        $("#airmail_supprimer a").click(function(ev){
            ev.preventDefault();
        ...
        $("#airmail_archiver a").click(function(ev){
            ev.preventDefault();
            var href = $(this).attr('href');
