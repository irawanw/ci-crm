...
<div class="row">
    <div class="col-md-2">
    	...
    	<a href="<?php echo site_url($controleur); ?>">Retour a la liste</a>
    </div>
    <br>
</div>
<div class="row">
...
<?php foreach ($cmd_locales as $c) {
    if ($c[2] == 'danger') {
        $ancre = anchor($c[1]."/$id",$c[0],'class="btn btn-danger" role="button"');
        $popup[$rang] = <<<EOD
    <div class="modal fade" id="popup_$rang" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
EOD;
        $cmd = '<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#popup_'.$rang.'">'.$c[0].'</button>'."&nbsp;\n";
        if (isset($c[3])) {
            if ($c[3]) echo $cmd;
        }
        else {
