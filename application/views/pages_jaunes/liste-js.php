...
<div id="modal-custom-remove" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la suppression du Pages Jaune
      </div>
...
<div id="modal-custom-archive" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la archive du Pages Jaune
      </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#datatable tbody').on('click', '.btn-view-detail', function(e) {
            ...
            if(isHide == false) {
                if ( row.child.isShown() ) {
                    ...
                    //get child data
                    $.post('<?php echo site_url(); ?>/pages_jaunes/index_child_json/0', {'parentId':tr_id}, function(data){
                        ...
                        if(data.data.length > 0) {
                            $.each(data.data, function(index, value){
                                ...
                                new_html = '<tr id="child-id-'+value.pages_jaunes_child_id+'" data-parent="'+tr_id+'" class="child child-'+tr_id+'">'+
                                ...
                                if ($('.child-'+tr_id).length == (index+1)) {
                                    ...
                                    $('#datatable tr').click(function(){
                                        ...
                                        $('#pages_jaunes_supprimer').addClass("action-confirm-danger");                            
                                        //modify button modification & remove
                                        if($(this).hasClass('child')){                                  
                                            child_id = $(this).attr('id').split('-');
                                            child_id = child_id[2];

                                            $("#pages_jaunes_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/pages_jaunes/modification_child/'+child_id);
                                            $("#pages_jaunes_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/pages_jaunes/archive_child/'+child_id);
                                            $("#pages_jaunes_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/pages_jaunes/remove_child/'+child_id);
                                            
                                        } else {
                                            $("#pages_jaunes_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/pages_jaunes/modification/'+parent_id);

                                            $("#pages_jaunes_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/pages_jaunes/remove/'+parent_id);

                                            $("#pages_jaunes_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/pages_jaunes/archive/'+parent_id);
                                        }
                                    })
                                }
                            })
                        } else {
                            new_html = '<tr data-parent="'+tr_id+'" class="child child-'+tr_id+'"><td colspan="28">None</td></tr>';
                            $(tr).after(new_html);
                        }
                    });             
                }
        ...
        $("#pages_jaunes_supprimer a").click(function(ev){
            ev.preventDefault();
        ...
        $("#pages_jaunes_archiver a").click(function(ev){
            ev.preventDefault();
            var href = $(this).attr('href');
