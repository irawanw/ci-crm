...
<?php }?>
    ...
    function dt_persist_state() {
        ...
        $('#datatable').DataTable().columns().every(function(cl){
			...
			/*
			 _cl_label = cl_head.find('div.movable').clone()    //clone the element
    ...
    $(document).ready(function() {
        ...
        $('#datatable').DataTable( {
            ...
            rowId: '<?php echo $descripteur['detail'][1]?>',
            ajax: {
            ...
            fnDrawCallback: function( settings ) {
                ...
                dt_reapply_column_widths();
                // Destroy and init again colResizable in order to
                // recreate resize handles for new table layout.
                $("#datatable").colResizable({ disable:true });
                setTimeout( function () {
                    dt_init_resizable();
                    $('#datatable_div').width($('#datatable.JColResizer').width());
                }, 200 );
                //console.log('.dataTables_scrollBody overflow is: '+$('.dataTables_scrollBody').css('overflow'));
                $('.dataTables_scrollBody').css('overflow-x','hidden');
                $('.dataTables_scrollBody').css('overflow-y','overlay');
                //console.log('.dataTables_scrollBody overflow-x is now set to: '+$('.dataTables_scrollBody').css('overflow-x'));
                //console.log('.dataTables_scrollBody overflow-y is now set to: '+$('.dataTables_scrollBody').css('overflow-y'));
                $('input.resizable').each(function (index, elem){
                    var el = $(elem);
...
<script>
 $("#check-all").click(function(e) {
  ...
  $("#btn_action_all").click(function(e) {
	etat_success='unsuccess';  
            var action = $("#sel_action_all" ).val();
              
           
               // var id = checkbox.val();
				var chkvalues = $('input:checkbox:checked.checkbox').map(function () {
					var id=this.value;
				                   if(action!='' && id!=''){ 
									renvoi(action,id);
									etat_success='success';
									}	
				  //return this.value;
				})
			...
			if(action!='' && etat_success=='success'){
				notificationWidget.show("Le des vigik a été modify", "success");
                ...
                $('#datatable_div').width($('#datatable.JColResizer').width());
				 ...
				 notificationWidget.show("Aucune action de l'état ou chèque pour l'envoi du vigik", "error");
			}
			// renvoi(action,record);
        });
    
	    // fonction de renvoi
        var renvoi = function(type, data) {
            $.ajax({
                method:"POST",
                url:"<?php echo site_url("newvigik/mass_action")?>",
                data: {"type":type,"id":data},
                success: function(reponse) {
                    if (reponse != false && data !='' && type !='') {
                      ...
                      //  notificationWidget.show("Le des vigik a été modify", "success");
                       ...
                       // notificationWidget.show("Pas d'etat check pour l'envoi du vigik", "error");
                    }
                },
                error: function() {
                    notificationWidget.show("Erreur lors du renvoi du vigik","error");
                }
            });
        };
...
<script>
  $("#btnjourn_action_all").click(function(e) {
	  etat_success='unsuccess';
            var action = $("#sel_action_all" ).val();
			...
			if(action!='' && etat_success=='success'){
				notificationWidget.show("Le des tournee journaliere  modify", "success");
                ...
                $('#datatable_div').width($('#datatable.JColResizer').width());
				 ...
				 notificationWidget.show("Aucune action de l'résultat de la livraison ou chèque pour l'envoi du tournee journaliere", "error");
			}
			// renvoi(action,record);
        });
        ...
        var renvoi = function(type, data) {
            $.ajax({
                method:"POST",
                url:"<?php echo site_url("newtournee_journalieres/mass_action")?>",
                data: {"type":type,"id":data},
                success: function(reponse) {
                    if (reponse != false && data !='' && type !='') {
                ...
                error: function() {
                    notificationWidget.show("Erreur lors du renvoi du tournee journaliere","error");
                }
            });
        };
