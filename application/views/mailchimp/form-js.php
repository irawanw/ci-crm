    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <div class="modal-body">
                êtes vous certain de vouloir annuler la saisie du Mail chimp ? 
            </div>
...
<div class="modal fade template-modal" id="modification_mailchimp" tabindex="-1" role="dialog" data-id="">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">				
				<div class="frm_modification_mailchimp">
...
<script>
    $(document).ready(function(){
	    ...
	    $("#modification_mailchimp").on("focus", "input.form-date-field",  function() {
	        $(this).datetimepicker({
	            format:'d/m/Y',
	            formatDate:'d/m/Y',
	            timepicker: false,
		...
		$("#template-modal-detail,#modification_mailchimp").on("change",'#client', function(){
			get_commande();			
		});
		...
		$("#modification_mailchimp").on('change',"#segment_numero", function() {
		...
		$('#template-modal-detail').on('shown.bs.modal',function(e) {	
			$('#form-mailchimp-nouveau-1').find('#segment_criteria').attr("readonly", true);					
			var formId = $(this).find('form').prop('id');	

			if(formId == "form-mailchimp-nouveau-1") {
				var btn = '<button type="button" class="btn btn-primary" id="enregister-suivant">Enregister / suivant</button>';
				var btnAnnuler = '&nbsp;<button class="btn btn-default" id="annuler-mailchimp">Annuler</button>';
				...
				var btnAnnulerChild = '&nbsp;<button class="btn btn-default" id="annuler-mailchimp-child">Annuler</button>';
				...
				setTimeout(function() {
					...
					if($("#annuler-mailchimp-child").length == 0) {
		...
		$('#template-modal-detail,#modification_mailchimp').on('hide.bs.modal',function(e) {	
			$('#enregister-suivant').remove();
			$('#enregister-suivant-child').remove();
			$('#annuler-mailchimp').remove();
			$('#annuler-mailchimp-child').remove();
		});
		...
		$(document).on("click","#modification_mailchimp .modal-body li a",function()
	    {
	        tab = $(this).attr("href");
		 ...
		 * FORM EDIT
		 */
		$('#modification_mailchimp').on('shown.bs.modal',function(e) {	
			$('#form-mailchimp-modification-1').find('#segment_criteria').attr("readonly", true);						
			var formId = $(this).find('form').prop('id');					

			if(formId == "form-mailchimp-modification-1") {
				var btn = '<button type="button" class="btn btn-primary" id="enregister-suivant">Enregister / suivant</button>';
				var btnAnnuler = '&nbsp;<button class="btn btn-default" id="annuler-mailchimp">Annuler</button>';

				$('#modification_mailchimp button[type="submit"]').hide();
				setTimeout(function() {
					if($("#enregister-suivant").length == 0) {
						$('#modification_mailchimp form p.text-center').append(btn);
					}
					if($("#annuler-mailchimp").length == 0) {
						$('#modification_mailchimp form p.text-center').append(btnAnnuler);
					}
				}, 20);
			} else {
				var btnChild = '<button type="button" class="btn btn-primary" id="enregister-suivant-child">Enregister / suivant</button>';
				...
				setTimeout(function() {
					if($("#enregister-suivant-child").length == 0) {
						$('#modification_mailchimp form p.text-center').append(btnChild);
					}
					if($("#annuler-mailchimp-child").length == 0) {
						$('#modification_mailchimp form p.text-center').append(btnAnnulerChild);
					}
				}, 20);
			}

			setTimeout(function() {
				get_segment_criteria($('#form-mailchimp-modification-1').find('#segment_numero').val(),true);
			},100);
		});

		$(document).on('click',"#modification_mailchimp #form-mailchimp-modification-1 #enregister-suivant",function(e) {
			var templateId = "#modification_mailchimp";
			...
			if(tabActive != "mailchimp-tab-4") {
				...
				$("#modification_mailchimp .modal-body .tab-content div").each(function(){
		            $(this).removeClass("active");
		        });
		        $("#modification_mailchimp #"+nextTab).addClass("active");
		...
		$(document).on('click',"#modification_mailchimp #enregister-suivant-child",function(e) {
			var templateId = "#modification_mailchimp";
			...
			if(tabActiveChild != "mailchimp-tab-5") {
		        ...
		        $("#modification_mailchimp #"+nextTabChild).addClass("active");
		...
		$('#modification_mailchimp').on('shown.bs.tab',"a",function(e) {
			var templateId = "#modification_mailchimp";
			var tab = $(templateId).find('ul.nav-tabs li.active a');
			var tabActive = $(tab).attr('aria-controls');
		...
		$('#modification_mailchimp').on('click',"#annuler-mailchimp,#annuler-mailchimp-child",function(e) {
			e.preventDefault();
			$('#modal-annuler-confirm').modal('show');
		});

		$('#mailchimp_modification a').click(function(e) {
			e.preventDefault();
			...
			$.ajax({
				...
				{
					$('.frm_modification_mailchimp').html(response.data);
					$('#modification_mailchimp').modal('show');			
				}
			});
		});
		 ...
		 * 
		 */
		$('#template-modal-detail').on("click","#form-submit-mailchimp-nouveau_child", function(e) {
			e.preventDefault();

			var form = $('#form-mailchimp-nouveau_child-1');			
		...
		$("#modification_mailchimp").on("click","#form-submit-mailchimp-modification,#form-submit-mailchimp-modification_child", function(e) {
			e.preventDefault();
		
			var form = $('#modification_mailchimp').find('form');			
			var url = $(form).attr("action");
	        var data = $(form).serialize();	   
	            	...
	            	if(response.success == true) {
	            		...
	            		if(event){
		            		...
		            		if(!isChild) {
		            			...
		            			$.ajax({
		            				type: 'POST',
		            				url: "<?php echo site_url('mailchimp/index_child_json');?>/" + event.id,
	            		...
	            		$('#modification_mailchimp').modal('hide');
	            	}
	...
	function reloadChildRow (id, subText, data) {
		$('#child-id-' + id).html('<td colspan="23">'+subText+'</td>'+				
                ...
                //'<td>'+data.mailchimp_number_of_open+'</td>'+
                //'<td>'+data.mailchimp_open_rate+'</td>'+
                '<td>'+data.mailchimp_number_of_click+'</td>'+
                '<td>'+data.mailchimp_click_rate+'</td>'+
	...
	function get_commande() {
		$.get("<?php echo site_url('mailchimp/commande_option');?>/"+$("#client").val(), function(data){
				$("#commande").html(data);
		});
	}	
	...
	function get_segment_criteria(segmentNumero, isUpdate) {
		$.get("<?php echo site_url('segments/get');?>/"+segmentNumero+"/criteria", function(response){
			if(isUpdate) {
				$("#form-mailchimp-modification-1").find('#segment_criteria').html(response.data);
			} else {
				$("#form-mailchimp-nouveau-1").find('#segment_criteria').html(response.data);
			}
		},"json");
	}
</script>
