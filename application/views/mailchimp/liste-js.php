...
<div id="modal-custom-remove" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la suppression du Mail chimp
      </div>
...
<div id="modal-custom-archive" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la archive du Mail chimp
      </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#datatable tbody').on('click', '.btn-view-detail', function(e) {
            ...
            if(isHide == false){
                if ( row.child.isShown() ) {
                    ...
                    //get child data
                    $.post('<?php echo site_url(); ?>/mailchimp/index_child_json/0', {'parentId':tr_id}, function(data){
                        ...
                        if(data.data.length > 0) {
                            $.each(data.data, function(index, value){                       
                                ...
                                new_html = '<tr id="child-id-'+value.mailchimp_child_id+'" data-parent="'+tr_id+'" class="child child-'+tr_id+'">'+
                                                ...
                                                //'<td>'+value.mailchimp_number_of_open+'</td>'+
                                                //'<td>'+value.mailchimp_open_rate+'</td>'+
                                                '<td>'+value.mailchimp_number_of_click+'</td>'+
                                                '<td>'+value.mailchimp_click_rate+'</td>'+
                                ...
                                if ($('.child-'+tr_id).length == (index+1)) {
                                    ...
                                    $('#datatable tr').click(function(){
                                        ...
                                        $('#mailchimp_supprimer').addClass("action-confirm-danger");                            
                                        //modify button modification & remove
                                        if($(this).hasClass('child')){                                  
                                            child_id = $(this).attr('id').split('-');
                                            child_id = child_id[2];

                                            $("#mailchimp_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/mailchimp/modification_child/'+child_id);
                                            $("#mailchimp_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/mailchimp/archive_child/'+child_id);
                                            $("#mailchimp_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/mailchimp/remove_child/'+child_id);
                                            
                                        } else {
                                            $("#mailchimp_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/mailchimp/modification/'+parent_id);

                                            $("#mailchimp_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/mailchimp/remove/'+parent_id);

                                            $("#mailchimp_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/mailchimp/archive/'+parent_id);
                                        }
                                    })
                                }
                            })
                        } else {
                            new_html = '<tr data-parent="'+tr_id+'" class="child child-'+tr_id+'"><td colspan="43">None</td></tr>';
                            $(tr).after(new_html);
                        }
                    });             
                }
        ...
        $("#mailchimp_supprimer a").click(function(ev){
            ev.preventDefault();
        ...
        $("#mailchimp_archiver a").click(function(ev){
            ev.preventDefault();
            var href = $(this).attr('href');
