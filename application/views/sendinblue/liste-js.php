...
<div id="modal-custom-remove" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la suppression du sendinblue
      </div>
...
<div id="modal-custom-archive" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la archive du sendinblue
      </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#datatable tbody').on('click', '.btn-view-detail', function(e) {
            ...
            if(isHide == false){
                if ( row.child.isShown() ) {
                    ...
                    //get child data
                    $.post('<?php echo site_url(); ?>/sendinblue/index_child_json/0', {'parentId':tr_id}, function(data){
                        ...
                        if(data.data.length > 0) {
                            $.each(data.data, function(index, value){                       
                                ...
                                new_html = '<tr id="child-id-'+value.sendinblue_child_id+'" data-parent="'+tr_id+'" class="child child-'+tr_id+'">'+
                                                ...
                                                '<td>'+value.sendinblue_number_of_click+'</td>'+
                                                '<td>'+value.sendinblue_click_rate+'</td>'+
                                ...
                                if ($('.child-'+tr_id).length == (index+1)) {
                                    ...
                                    $('#datatable tr').click(function(){
                                        ...
                                        $('#sendinblue_supprimer').addClass("action-confirm-danger");                            
                                        //modify button modification & remove
                                        if($(this).hasClass('child')){                                  
                                            child_id = $(this).attr('id').split('-');
                                            child_id = child_id[2];

                                            $("#sendinblue_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/sendinblue/modification_child/'+child_id);
                                            $("#sendinblue_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/sendinblue/archive_child/'+child_id);
                                            $("#sendinblue_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/sendinblue/remove_child/'+child_id);
                                            
                                        } else {
                                            $("#sendinblue_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/sendinblue/modification/'+parent_id);

                                            $("#sendinblue_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/sendinblue/remove/'+parent_id);

                                            $("#sendinblue_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/sendinblue/archive/'+parent_id);
                                        }
                                    })
                                }
                            })
        ...
        $("#sendinblue_supprimer a").click(function(ev){
            ev.preventDefault();
        ...
        $("#sendinblue_archiver a").click(function(ev){
            ev.preventDefault();
            var href = $(this).attr('href');
