...
<div id="modal-custom-remove" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la suppression du Openemm
      </div>
...
<div id="modal-custom-archive" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la archive du Openemm
      </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#datatable tbody').on('click', '.btn-view-detail', function(e) {
            ...
            if(isHide == false){
                if ( row.child.isShown() ) {
                    ...
                    //get child data
                    $.post('<?php echo site_url(); ?>/openemm/index_child_json/0', {'parentId':tr_id}, function(data){
                        ...
                        if(data.data.length > 0) {
                            $.each(data.data, function(index, value){                       
                                ...
                                new_html = '<tr id="child-id-'+value.openemm_child_id+'" data-parent="'+tr_id+'" class="child child-'+tr_id+'">'+
                                                ...
                                                /*'<td>'+value.openemm_number_of_open+'</td>'+
                                                '<td>'+value.openemm_open_rate+'</td>'+*/
                                                '<td>'+value.openemm_number_of_click+'</td>'+
                                                '<td>'+value.openemm_click_rate+'</td>'+
                                ...
                                if ($('.child-'+tr_id).length == (index+1)) {
                                    ...
                                    $('#datatable tr').click(function(){
                                        ...
                                        $('#openemm_supprimer').addClass("action-confirm-danger");                            
                                        //modify button modification & remove
                                        if($(this).hasClass('child')){                                  
                                            child_id = $(this).attr('id').split('-');
                                            child_id = child_id[2];

                                            $("#openemm_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/openemm/modification_child/'+child_id);
                                            $("#openemm_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/openemm/archive_child/'+child_id);
                                            $("#openemm_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/openemm/remove_child/'+child_id);
                                            
                                        } else {
                                            $("#openemm_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/openemm/modification/'+parent_id);

                                            $("#openemm_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/openemm/remove/'+parent_id);

                                            $("#openemm_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/openemm/archive/'+parent_id);
                                        }
                                    })
                                }
                            })
        ...
        $("#openemm_supprimer a").click(function(ev){
            ev.preventDefault();
        ...
        $("#openemm_archiver a").click(function(ev){
            ev.preventDefault();
            var href = $(this).attr('href');
