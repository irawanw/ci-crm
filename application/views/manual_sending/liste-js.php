    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Form View Long Text</h4>
            ...
            <form method="post" action="<?php echo site_url('manual_sending/update_value');?>">
...
<div id="modal-custom-remove" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la suppression du manual sending
      </div>
...
<div id="modal-custom-archive" class="modal fade" role="dialog">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            Veuillez confirmer la archive du manual sending
      </div>
...
<script>
    $(document).ready(function() {
          ...
          e.preventDefault();
            ...
            if(isHide == false) {
                if ( row.child.isShown() ) {
    				...
    				//get child data
    				$.post('<?php echo base_url(); ?>index.php/manual_sending/index_child_json/0', {'parentId':tr_id}, function(data){
                        ...
                        if(data.data.length > 0) {
        					$.each(data.data, function(index, value){
        						...
        						new_html = '<tr id="child-id-'+value.manual_sending_child_id+'" data-parent="'+tr_id+'" class="child child-'+tr_id+'">'+
        										...
        										'<td>'+value.manual_sender_email+'</td>'+
        						...
        						if ($('.child-'+tr_id).length == (index+1)) {
        							...
        							$('#datatable tr').click(function(){
                                        ...
                                        $('#manual_sending_supprimer').addClass("action-confirm-danger");                            
                                        //modify button modification & remove
        								if($(this).hasClass('child')){									
        									child_id = $(this).attr('id').split('-');
        									child_id = child_id[2];

        									$("#manual_sending_modification a").attr('href', 
        										'<?php echo site_url(); ?>/manual_sending/modification_child/'+child_id);
                                            $("#manual_sending_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/manual_sending/remove_child/'+child_id);
                                            $("#manual_sending_archive a").attr('href', 
                                                '<?php echo site_url(); ?>/manual_sending/archive_child/'+child_id);
                                            
        								} else {
        									$("#manual_sending_modification a").attr('href', 
        										'<?php echo site_url(); ?>/manual_sending/modification/'+parent_id);

                                            $("#manual_sending_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/manual_sending/remove/'+parent_id);
                                            $("#manual_sending_archive a").attr('href', 
                                                '<?php echo site_url(); ?>/manual_sending/archive/'+parent_id);
        								}
        							})
        						}
        					})
                        } else {
                            new_html = '<tr data-parent="'+tr_id+'" class="child child-'+tr_id+'"><td colspan="46">None</td></tr>';
                            $(tr).after(new_html);
                        }
    				});				
                }
        ...
        $("#manual_sending_supprimer a").click(function(ev){
            ev.preventDefault();
        ...
        $("#manual_sending_archive a").click(function(ev){
            ev.preventDefault();
            var href = $(this).attr('href');
