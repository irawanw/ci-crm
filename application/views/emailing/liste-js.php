...
<script>
    $(document).ready(function() {
        ...
        $('#datatable tbody').on('click', '.btn-view-detail', function(e) {
            ...
            if(isHide == false){
                if ( row.child.isShown() ) {
                    ...
                    //get child data
                    $.post('<?php echo site_url(); ?>/emailing/index_child_json/0', {'parentId':tr_id}, function(data){
                        ...
                        if(data.data.length > 0) {
                            $.each(data.data, function(index, value){                       
                                ...
                                new_html = '<tr id="child-id-'+value.emailing_child_id+'" data-parent="'+tr_id+'" class="child child-'+tr_id+'">'+
                                ...
                                if ($('.child-'+tr_id).length == (index+1)) {
                                    ...
                                    $('#datatable tr').click(function(){
                                        ...
                                        $('#emailing_supprimer').addClass("action-confirm-danger");                            
                                        //modify button modification & remove
                                        if($(this).hasClass('child')){                                  
                                            child_id = $(this).attr('id').split('-');
                                            child_id = child_id[2];

                                            $("#emailing_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/emailing/modification_child/'+child_id);
                                            $("#emailing_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/emailing/archive_child/'+child_id);
                                            $("#emailing_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/emailing/remove_child/'+child_id);
                                            
                                        } else {
                                            $("#emailing_modification a").attr('href', 
                                                '<?php echo site_url(); ?>/emailing/modification/'+parent_id);

                                            $("#emailing_supprimer a").attr('href', 
                                                '<?php echo site_url(); ?>/emailing/remove/'+parent_id);

                                            $("#emailing_archiver a").attr('href', 
                                                '<?php echo site_url(); ?>/emailing/archive/'+parent_id);
                                        }
                                    })
                                }
                            })
        ...
        $("#emailing_supprimer a").click(function(ev){
            ev.preventDefault();
        ...
        $("#emailing_archiver a").click(function(ev){
            ev.preventDefault();
            var href = $(this).attr('href');
